...
for shape in shapes: 
    for i in range(-0.95, 0.01, 0.95):
        for j in range(-0.95,0.01, 0.95):
            ...
            c += 1
            hv.save(shape[:], 'illusion_image/blur/{}.png'.format(c))
            pat = Image.open('illusion_image/blur/{}.png'.format(c))
            ill_0 = illusion(pat)
            ill_0.save('illusion_image/blur/{}.png'.format(c))

            c += 1
            pat = pat.resize((16, 16))
            pat = pat.crop((1,1,15,15))
            ill_1 = illusion(pat)
            ill_1.save('illusion_image/blur/{}.png'.format(c))
